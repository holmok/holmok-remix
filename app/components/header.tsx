    ...
    <div className='header'>
      <div className='container'>
        ...
        {(props.hideNav == null || !props.hideNav) && (
          <>
            <div className='nav'><a href='#' className={menuOpen ? 'close' : 'open'} onClick={toggleMenu}>{menuOpen ? '✕' : '☰'}</a>
            </div>
            {menuOpen && <Menu closeMenu={closeMenu} />}
          </>)}
        {props.hideNav != null && props.hideNav && (<div className='nav'><a href='/'>Return Home</a></div>)}
      </div>
    </div>
  )
}
